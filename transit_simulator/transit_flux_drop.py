...
def transit_flux_drop(limb_darkening_coefficients, rp_over_rs, z_over_rs, method='claret', precision=3):
    ...
    if len(case0[0]) > 0:
        plusflux[case0] = integral_centred(limb_darkening_coefficients, rp_over_rs, 0.0, np.pi, method=method)
    if len(caseb[0]) > 0:
        plusflux[caseb] = integral_centred(limb_darkening_coefficients, 1, 0.0, np.pi, method=method)
    ...
    # flux_total
    total_flux = integral_centred(limb_darkening_coefficients, 1, 0.0, 2.0 * np.pi, method=method)
